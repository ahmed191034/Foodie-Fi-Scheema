...
-- Optional: clear existing rows if you re-run
...
SELECT 
    COUNT(*) AS churn_after_trial_customers,
    
    ROUND(
        COUNT(*) * 100.0 /
        (SELECT COUNT(DISTINCT customer_id) FROM subscriptions),
    0) AS percentage_of_customers
FROM ordered_plans
WHERE plan_order = 2
AND plan_id = 4;
...
# Whose annual plan started in 2020
SELECT 
...
SELECT
    CONCAT(bucket_start, '-', bucket_end) AS day_period,
    COUNT(*) AS customers,
    ROUND(COUNT(*) * 100.0 / total.total_customers, 1) AS pct_customers
FROM (
    SELECT
        FLOOR(days_to_annual / 30) * 30 + 1 AS bucket_start,
        FLOOR(days_to_annual / 30) * 30 + 30 AS bucket_end
    FROM annual_lags
) buckets
CROSS JOIN (
    SELECT COUNT(*) AS total_customers FROM annual_lags
) total
GROUP BY bucket_start, bucket_end, total.total_customers
